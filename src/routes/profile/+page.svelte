<script lang="ts">
	...
	async function loadPinnedNotes(nextPubkey: string) {
		...
		if (loadedFor !== nextPubkey) return;
		pinnedNotes = applyActivityToNotes(ordered, activity, me?.pk);
	}
	...
	async function loadInteractionNotes(nextPubkey: string) {
		...
		const hydrate = async (targetIds: string[]) => {
			...
			return applyActivityToNotes(ordered, activity, me?.pk);
		};
			...
			<div class="flex gap-1">
				{#each ['posts', 'replies', 'media'] as tab (tab)}
					<button
						type="button"
						onclick={() =>
							(activeTab = tab as 'posts' | 'replies' | 'media' | 'pinned' | 'liked' | 'reposts')}
