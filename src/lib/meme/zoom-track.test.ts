...
describe('normalizeZoomWindow', () => {
	it('accepts a well-formed window and rounds ms', () => {
		expect(
			normalizeZoomWindow({ startMs: 100.6, endMs: 900.4, factor: 1.6, cx: 0.4, cy: 0.3 })
...
describe('zoomActiveAt / zoomTransformAt', () => {
	const zooms = [{ startMs: 1000, endMs: 2000, factor: 2.5, cx: 0.7, cy: 0.3 }];
...
describe('composeZoomWithFraming', () => {
	it('multiples scales and sums pans, clamped', () => {
		expect(
			composeZoomWithFraming({ scale: 1.5, x: 0.2, y: -0.2 }, { scale: 2, x: 0.6, y: -0.6 })
		).toEqual({ scale: 3, x: 0.8, y: -0.8 });
		expect(composeZoomWithFraming({ scale: 3.5, x: 0, y: 0 }, { scale: 2, x: 0, y: 0 }).scale).toBe(
			4
		);
	});
