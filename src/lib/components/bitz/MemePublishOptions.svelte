...
{#if open}
	...
	<div class="fixed inset-0 z-[100] grid h-full w-full place-items-center p-3 sm:p-5">
		...
		>
			<header class="flex shrink-0 items-start justify-between gap-3 border-b border-[var(--ui-border-muted)] p-4 sm:p-5"><div><h2 id="publish-details-title" class="text-base font-bold">Public publishing details</h2><p class="mt-1 text-[12px] text-[var(--ui-text-muted)]">Review your public post, settings and destinations before publishing.</p></div><button type="button" onclick={() => (open = false)} disabled={busy} aria-label="Close publishing details" class="rounded-full p-1 text-[var(--ui-text-muted)] hover:bg-[var(--ui-bg-muted)] disabled:opacity-40"><Icon name="i-lucide-x" class="size-4" /></button></header>
			<div class="scrollbar-thin min-h-0 space-y-5 overflow-y-auto p-4 sm:p-5">
				...
				<section><h3 class="text-[12px] font-bold">{noteOnly ? 'Note text' : 'Public description'}</h3><p class="mt-0.5 text-[11px] text-[var(--ui-text-muted)]">{noteOnly ? 'This will be published as a regular Nostr note (kind 1), with your meme attached.' : 'This accompanies the finished meme in each selected destination.'}</p><div class="mt-2"><MemePostCaption bind:value={caption} {busy} softLimit={softCaptionLimit} hardLimit={hardCaptionLimit} /></div></section>
				...
				<section>
					...
					<p class="mt-0.5 text-[11px] text-[var(--ui-text-muted)]">
						Choose where the public meme file is stored. This does not change where the Nostr post is published.
					</p>
					<Popover id={providerMenuId} placement="top-start" width="lg" label="Upload provider" triggerClass="mt-2 flex items-center gap-1.5 rounded-full px-3 py-2 text-[12px] font-semibold text-[var(--ui-text-muted)] transition hover:bg-[var(--ui-bg-muted)] hover:text-[var(--ui-text)]" triggerActiveClass="bg-primary-500/10 text-primary-600">
						{#snippet trigger()}<Icon name="i-lucide-cloud-upload" class="size-[15px] text-primary-500" /><span class="max-w-[120px] truncate">{selectedProviderLabel}</span>{/snippet}
						<MenuItem icon="i-lucide-hard-drive-upload" onclick={() => (selectedProvider = 'none')} tone={selectedProvider === 'none' ? 'accent' : 'default'}>
							BitOS uploads
							{#snippet trailing()}{#if selectedProvider === 'none'}<Icon name="i-lucide-check" class="size-4 shrink-0" />{/if}{/snippet}
						</MenuItem>
						<MenuDivider />
						{#each MEDIA_PROVIDERS as provider (provider.id)}
							<MenuItem icon={provider.icon} disabled={!media.isConfigured(provider.id)} tone={selectedProvider === provider.id ? 'accent' : 'default'} onclick={() => (selectedProvider = provider.id)}>
								<div class="min-w-0"><div>{provider.label}</div><div class="text-[11px] font-medium text-[var(--ui-text-dimmed)]">{media.isConfigured(provider.id) ? provider.description : 'Configure this provider in Settings first'}</div></div>
								{#snippet trailing()}{#if selectedProvider === provider.id}<Icon name="i-lucide-check" class="size-4 shrink-0" />{/if}{/snippet}
							</MenuItem>
						{/each}
					</Popover>
				</section>
			</div>
			<footer class="flex shrink-0 items-center justify-between gap-3 border-t border-[var(--ui-border-muted)] p-4 sm:px-5"><p class="flex min-w-0 items-center gap-1.5 text-[10.5px] text-[var(--ui-text-dimmed)]"><Icon name="i-lucide-globe" class="size-3.5 shrink-0 text-primary-500" />Public to {writeRelayCount} {writeRelayCount === 1 ? 'relay' : 'relays'} · {noteOnly ? 'kind 1 note' : (kindNip ?? 'Nostr')}</p><div class="flex items-center gap-2"><button type="button" onclick={() => (open = false)} disabled={busy} class="rounded-full px-3 py-2 text-[12px] font-bold text-[var(--ui-text-muted)] disabled:opacity-40">Keep editing</button><button type="button" onclick={() => { open = false; onPublish(); }} disabled={busy || !splitCheck.ok} class="rounded-full bg-warm-500 px-4 py-2 text-[12px] font-bold text-white disabled:opacity-40"><Icon name="i-lucide-send" class="mr-1 inline size-3.5" />{noteOnly ? 'Post note' : 'Publish publicly'}</button></div></footer>
		</div>
	</div>
{/if}
