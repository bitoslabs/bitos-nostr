		...
		{:else if trending.length}
			<div class="space-y-2">
				{#each trending as t (t.tag)}
					<button
						type="button"
						onclick={() => toasts.info(`Exploring ${t.tag}`)}
				...
				{#each suggested as s (s.pubkey)}
					...
					<div class="flex items-center gap-3">
						...
						<button
							type="button"
							onclick={() => {
								following[s.pubkey] = !following[s.pubkey];
								following = { ...following };
							}}
							class="rounded-full px-3 py-1.5 text-[11px] font-bold transition-colors {following[
								s.pubkey
							]
								? 'bg-[var(--ui-bg-accented)] text-[var(--ui-text-muted)]'
								: 'bg-primary-500 text-white hover:bg-primary-600'}"
						>
							{following[s.pubkey] ? 'Following' : 'Follow'}
						</button>
					</div>
				{/each}
			</div>
