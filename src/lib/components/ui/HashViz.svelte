<script lang="ts">
	...
	let {
		...
		length = 32,
		class: cls
	}: { bits?: number; length?: number; class?: string } = $props();

	const segments = $derived(
		Array.from({ length }, (_, i) => i < Math.min(bits, length))
	);
</script>

<span class={cn('hash-viz', cls)} aria-label="{bits} leading bits" role="img">
