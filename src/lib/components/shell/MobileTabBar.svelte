<script lang="ts">
	...
	// Immersive full-screen editors own the whole viewport — the tab bar
	// hides there (native-app behavior; docs/studio-mobile-ux.md §2).
	const immersive = $derived(
		page.url.pathname === '/studio/create' || page.url.pathname.startsWith('/studio/create/')
	);
</script>
